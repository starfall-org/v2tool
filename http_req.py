...
def get_response(url):
    ...
    if any(proto in response for proto in ["vmess:", "trojan:", "vless:"]):
        for line in response.splitlines():
            if any(proto in line for proto in ["vmess:", "trojan:", "vless:"]):
                links.append(line)
    elif any(proto in response for proto in ["http:", "https:"]):
        url_pattern = r'https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+'
        sub_urls = re.findall(url_pattern, response)
        links.extend(get_responses(sub_urls))
    else:
        decoded_line = base64.b64decode(response.encode()).decode("utf-8")
        for line in decoded_line.splitlines():
            if any(proto in line for proto in ["vmess:", "trojan:", "vless:"]):
                links.append(line)
    return links
